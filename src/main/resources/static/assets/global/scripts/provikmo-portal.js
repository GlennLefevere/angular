...
var Portal = function() {
	...
	var fixProvikmoLayout = function() {
		
		$("#clientzone> form").appendTo("#authenticationlist");
		
		$(".item-list.rFloat > ul.menu > li.last").children().appendTo(
				"#authenticationlist > form");
		
		$("#clientzone").remove();
		
		$(".item-list.rFloat").remove();
		
		if ($("#authenticationlist > form > a > b").text().toUpperCase() === "inloggen"
				.toUpperCase()) {
			$("#authenticationlist > form > a > b").remove();
			$("#authenticationlist > form > a").append(
					"<span class='icon-login'></span>");
			$("#authenticationlist > form > a").addClass("authentication");
		} else {
			$("#authenticationlist > form > a").text("");
			$("#authenticationlist > form > a").append(
				...
				.on(
						...
						function() {
							if ($(this).children().first().attr("class")
									.indexOf("icon-size-fullscreen") >= 0) {
								screenfull.request();
		...
		shortcut.add("f11", function(e){
			if($(".resize > a").children().first().attr("class").indexOf("icon-size-fullscreen") >= 0){
				screenfull.request();
				$(".resize > a").children().first().removeClass(
						"icon-size-fullscreen");
				$(".resize > a").children().first().addClass(
						"icon-size-actual");
			}else{
				screenfull.exit();
				$(".resize > a").children().first().removeClass(
		...
		$(".page-sidebar-menu > li").each(function(){
			if($(this).find("ul").size() > 0 && $(this).find("li").size() == 0){
				$(this).remove();
			}
		});
	};
	...
	var handleScrollers = function() {
		Portal.initSlimScroll('.scroller');
	};
	...
	return {
		...
		alert : function(options) {
			...
			var id = Portal.getUniqueID("Metronic_alert");

			var html = '<div id="'
					+ id
					+ '" class="Portal-alerts alert alert-'
					+ options.type
					+ ' fade in">'
